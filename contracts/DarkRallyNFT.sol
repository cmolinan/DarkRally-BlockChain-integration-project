    ...
    UUPSUpgradeable
{
    ...
    function registerNewTypeOfNft(
        uint256 tokenId,
        string calldata nameOfNFT,
        string calldata category,
        string calldata metadataHashIpfs,
        uint256 maxSupply,
        bool askDateForMint,
        uint256 validUntil
    ) public onlyRole(BUSINESS_ROLE) whenPaused {
        require(
            !nftInfo[tokenId].tokenIsRegistered,
            "TokenId was already registered"
        );
        require(
            bytes(metadataHashIpfs).length > 32,
            "Check the MetadataHashIPFS entry"
        );
     ...
     */
    function deleteRegisterOfTypeOfNft(
        uint256 tokenId
    ...
    function changeMetadataHashOfNft(
        ...
        require(
            bytes(_metadataHashIpfs).length > 0,
            "MetadataHash can't be empty"
        );
    ...
    function changeMaxSupplyOfNft(
        ...
        require(nftInfo[_tokenId].tokenIsRegistered, "Token is not registered");
        require(
            _maxSupply != 0 && nftInfo[_tokenId].maxSupply != _maxSupply,
            "Nothing to change"
        );
    ...
    function mint(
        ...
        require(
            totalSupply(tokenId) + amount <= nftInfo[tokenId].maxSupply,
            "Maximum supply has been reached"
        );
        // solhint-disable-next-line
        if (nftInfo[tokenId].askDateForMint)
            require(
                nftInfo[tokenId].validUntil > block.timestamp,
    ...
    function uri(
        ...
        return (
            string(
                abi.encodePacked(
                    "https://ipfs.io/ipfs/",
                    nftInfo[_tokenId].metadataHashIpfs
                )
            )
        );
    }
