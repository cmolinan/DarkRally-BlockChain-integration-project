    ...
    UUPSUpgradeable
{
    ...
    function createSaleOffer(
        ...
        require(
            darkRallyNFTSC.balanceOf(msg.sender, _tokenId) >= _quantity,
            "Don't own that amount of tokens"
        );

        forSaleUuid.push(uuid);

        forSaleInfo[uuid] = ForSaleInfo(
            _tokenId,
            msg.sender,
            _price,
            _quantity,
            forSaleUuid.length - 1,
            true
        );
    }
    ...
    function purchaseNft(
        ...
        require(
            darkRallyNFTSC.balanceOf(_owner, _tokenId) >= _quantity,
            "Owner don't have enough tokens"
        );
        ...
        require(
            forSaleInfo[uuid].price == _price,
            "Registered price is different"
        );

        uint256 maxQuantityToSale = forSaleInfo[uuid].quantity;
        require(
            _quantity != 0 && maxQuantityToSale >= _quantity,
            "Amount is not authorized to sale"
        );

        // verify USDC Coin
        require(
            usdCoinSC.allowance(msg.sender, address(this)) >= _price,
            "Not enough USDC allowance"
        );
        require(
            usdCoinSC.balanceOf(msg.sender) >= _price,
        ...
        if (maxQuantityToSale - _quantity > 0) {
            ...
            forSaleUuid[forSaleInfo[uuid].arrayIndex] = forSaleUuid[
                forSaleUuid.length - 1
            ];
        ...
        require(
            usdCoinSC.transferFrom(msg.sender, _owner, net),
            "Transfer to Owner wallet failed"
        );

        // transfer coins to company Wallet
        require(
            usdCoinSC.transferFrom(
                msg.sender,
                scAddresses.companyWallet,
                company
            ),
        ...
        darkRallyNFTSC.safeTransferFrom(
            _owner,
            msg.sender,
            _tokenId,
            _quantity,
    ...
    function getSalesList() external view returns (string[] memory) {
        ...
        for (uint256 i = 0; i < forSaleUuid.length; ++i) {
            ...
            infoSales = string.concat(
                Strings.toString(forSaleInfo[uuid].tokenId),
                "_",
                Strings.toHexString(uint160(forSaleInfo[uuid].owner), 20),
                "_",
                Strings.toString(forSaleInfo[uuid].price),
                "_",
    ...
    function updatePriceAndQuantity(
        ...
        bytes32 uuid = keccak256(abi.encodePacked(_tokenId, "_", msg.sender));
        require(
            forSaleInfo[uuid].isRegistered,
            "Token not registered for Sale"
        );
        require(
            _newPrice != forSaleInfo[uuid].price ||
                _newQuantity != forSaleInfo[uuid].quantity,
            "Nothing to change"
        );
        require(
            darkRallyNFTSC.balanceOf(msg.sender, _tokenId) >= _newQuantity,
            "Owner don't have enough tokens"
        );
